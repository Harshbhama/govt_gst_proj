...
const anx13hDao= require("../../dao/anx1Dao/anx13hDao");
...
async function saveTab3HTxn(req,res) {
  logger.log("info","Entering anx13hService.saveTab3HTxn");
    ...
    //   return new Promise((resolve, reject) => {
        ...
        let save3h = await anx13hDao.savetab3HTxn(headerList ,items , gstin, fp, rtnprd)
...
async function editTab3HTxn(req,res) {
  logger.log("info","Entering anx13hService.editTab3HTxn");
  try {
    let gstin = req.headers["gstin"] || null;
  let rtnprd = req.headers["rtnprd"] || null;
  let fp = req.headers["fp"] || null;
  ...
  if (!validate3h(req, res)) {
    ...
    // return new Promise((resolve, reject) => {
      ...
      let edit3h = await anx13hDao.editTab3HTxn(editTab3h, items , gstin, req.headers["flag"])
...
if(validateHeaders){
  let getData = await anx13hDao.gettable3HdetailsTxn(req.headers["gstin"] , rtnprd , flag)
 ...
 * @param {*} res
 */
async function deleteTab3HByItemIdTxn(req, res) {
...
try {
  if (validateHeaders(req, res)) {
    ...
    if ( !req.body || !req.body.itemids || !Array.isArray(req.body.itemids) || _.isEmpty(req.body.itemids) ) {
     ...
     anx13hDao.deleteTab3HByItemIdTxn(req.body.itemids, req.headers["gstin"], fp, rtnprd)
     .then(data => {
  ...
  if (validateHeaders(req, res)) {
    ...
    if (!req.body || !req.body.docids || !Array.isArray(req.body.docids) || req.body.docids === null) {
      sendResponse(res, 500, { message: "Missing docIds array in body", statusCd: 0 });
        ...
        let data = await anx13hDao.deleteTab3HBydocidsTxn(req.body.docids, req.headers["gstin"], fp, rtnprd, req.headers["flag"],req.body.action)
...
module.exports = {
    ...
    delete3HItemsByItemIdTxn: deleteTab3HByItemIdTxn
