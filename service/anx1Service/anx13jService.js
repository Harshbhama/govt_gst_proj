...
const { validate3j } = require("../../utility/validators/anx1Validators/3jValidator");
const anx13jDao = require("../../dao/anx1Dao/anx13jDao");
 ...
 * @param {*} res 
 */
async function getTab3jdetails(req, res) {
  ...
  if (validateHeaders(req, res)) {
    ...
    let getAll3j = await anx13jDao.getTab3jdetails(gstin, rtnprd , flag)
 ...
 * @param {*} res
*/
async function saveTab3jTxn(req, res) {
  logger.log("info","Entering anx13jService.saveTab3jTxn");
  try{
    if (validateHeaders(req, res) && validate3j(req,res)) {
      if (_.isEmpty(req.body) || _.isEmpty(req.body.impg[0]) || _.isEmpty(req.body.impg[0].docs)) {
        ...
        let validateItems = await req.body.impg[0].docs[0].items.map( row => {       
        ...
        if(validateItems[0] === 1){
          ...
          let doc = req.body.impg[0].docs[0]
            let tab3j = {
              "docnum":doc.boe.num,
              "doctyp":doc.doctyp,
              "pcode": doc.boe.pcode,
            ...
            let items = await req.body.impg[0].docs[0].items.map(row => {
            ...
            let save3j = await anx13jDao.saveTab3jTxn(tab3j, items, gstin)
 ...
 * @param {*} res
*/
async function editTab3jTxn(req, res) {
    ...
    if (validateHeaders(req, res) && validate3j(req,res)) {
      if (_.isEmpty(req.body) || _.isEmpty(req.body.impg[0]) || _.isEmpty(req.body.impg[0].docs)) {
        ...
        let lclpos = doc.docref.slice(0, 2)
          ...
          let edit3j = await anx13jDao.editTab3jTxn(tab3j, items, gstin,req.headers["flag"],doc.flag)
...
* @param {*} res
*/
async function deleteTab3jBydocidsTxn(req, res) {
  ...
  try {
    if (validateHeaders(req, res)) {
      ...
      if (_.isEmpty(req.body) || _.isEmpty(req.body.docids)) {
        ...
        let delete3jData = await anx13jDao.deleteTab3jBydocidsTxn(req.body.docids, gstin, fp, rtnprd , req.headers["flag"], req.body.action)          
 ...
 * @param {*} res
 */
function deleteTab3jByItemIdTxn(req, res) {
  ...
  try {
    if (validateHeaders(req, res)) {
      ...
      if ( !req.body || !req.body.itemids || !Array.isArray(req.body.itemids) || _.isEmpty(req.body.itemids) ) {
        ...
        anx13jDao.deleteTab3jByItemIdTxn(req.body.itemids, req.headers["gstin"], fp, rtnprd,req.headers['flag'] , req.body.status)
          .then(data => {
