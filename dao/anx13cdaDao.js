...
function save3cda(headerList ,fullItemList , gstin){
    return new Promise((resolve, reject) => {
        ...
        db.save(
          anx1Queries.table3CDA.save3cdA,
          [ headerList.doctyp,
            gstin,
            headerList.docref,
            headerList.num,
    ...
    async function edit3cda(headerList ,fullItemList , gstin , fp , taxperiod) {
      ...
      try {
        ...
        let update3cd = await db.update(
          anx1Queries.table3CDA.update3cd,
          [ 
            headerList.doctyp,
            headerList.docref,
            headerList.num,
...
async function get3CDADetailsDao(rtnprd, dbName, flag) {
  ...
  try {
    ...
    */
   let sql = anx1Queries.table3CDA.get3cdAByReturnPeriod
   if(flag == "F") sql  = anx1Queries.table3CDA.get3cdAByReturnPeriodErr
...
async function delete3cdaBydocIds(docIds, dbName, fp, rtnprd) {
  try {
    ...
    let docIdsResult = await db.fetchAllById(
      anx1Queries.table3CDA.get3cdAByDocIds,
      docIds,
      dbName,
      true
    );
  ...
  try {
   ...
   let deleteRecords = 0
    ...
    let row3CD = await db.fetchAllById(
      anx1Queries.table3CDA.get3cdADetailsByDocRef,
      [item[0].ITEMREF],
      dbName
    );
    ...
    let recordsUpdated = await db.update(
      anx1Queries.table3CDA.update3cdADetailsByDocRef,
      [u.TAXVAL, u.IGST, u.CESS, u.DOCREF],
      dbName
    );
    
  db.fetchById(anx1Queries.table3CDA.get3cdADetailsByDocRef , [u.DOCREF] , dbName)
  .then(data =>{
...
async function delete3cdaItemsByItemIdTxn(itemIds = [], gstin, fp, taxperiod , flag , status) {
  ...
  try {
    ...
    // fetch row from 3CD table based on docRef
    let row3CD = await db.fetchAllByIdTxn(anx1Queries.table3CDA.get3cdADetailsByDocRef, [item[0].ITEMREF], gstindb);
    ...
    // update record DB call
    let recordsUpdated = await db.updateOrDeleteTxn(anx1Queries.table3CDA.update3cdADetailsByDocRef
      , [u.TAXVAL, u.IGST, u.CESS, u.DOCREF]
      , gstindb
    );
    if(status && status === "F"){
      await db.updateRow(anx1Queries.table3CDA.update3cdAFlagByDocRef, [u.DOCREF], gstindb);  // to update flag as corrected
    } 
...
function getDetailsByExportType(rtnprd ,exptyp ,gstin) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    db.fetchAllById(anx1Queries.table3CDA.getDetailsByExpTyp,
          [rtnprd ,  exptyp],   gstin)
      .then(resultSet => {
...
function get3CDajSON(rtnprd ,exptyp ,gstin,flag){
  return new Promise(function (resolve, reject) {
    ...
    let sql = anx1Queries.table3CDA.get3CDAjSON;
    if (flag == "F")
      sql = anx1Queries.table3CDA.get3cdAErrJson;
...
async function save3cdaTxn(tab3cd ,items , gstin) {
  ...
  try {
           ...
           await db.saveTxn(anx1Queries.table3CDA.save3cdA,
            [ tab3cd.doctyp,
              gstin,
              tab3cd.docref,
              tab3cd.num,
    ...
    if (docIdsResult.length === 0) {
      ...
      if (diffRec.length === 0) {
        ...
        let deleteRecords = await db.deleteBySqlInClauseTxn(
          anx1Queries.table3CDA.delete3cdAByDocIds,
          docIds,
          gstindb 
               );
...
async function summary3cdTxn(fp, taxperiod, gstindb, isMFD = false) {
  try {
    let returnValue = 1
    await db.saveTxn(anx1Queries.summary.delete3Aor3Hsumm, [fp, taxperiod, "3CDA"], gstindb);
  ...
  try {
    ...
    if(flag && flag =="N"){ // To update summary when corrected record is deleted in main flow
      if(actionFlag && actionFlag =="C"){
        await db.saveTxn(anx1Queries.errorSummary.delete3Aor3Hsumm, [fp, taxperiod, "3CDA"], gstindb);
        ...
        if (mfd.length === 0 && isMFD) {
          ...
          let row = await db.fetchByIdTxn(anx1Queries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
          if (row) {
      ...
      if (mfd.length === 0 && isMFD) {
        ...
        await db.saveTxn(anx1Queries.errorSummary.tableSummaryAwpAndwopNet, [fp, taxperiod, fp, taxperiod], gstindb);
        //await db.saveTxn(anx1Queries.errorSummary.consolidateNetForDocTypes, [fp, taxperiod], gstindb);
        let row = await db.fetchByIdTxn(anx1Queries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
        if (row) {
          await db.saveTxn(anx1Queries.errorSummary.consolidateNet, [fp, taxperiod, fp, taxperiod], gstindb)
