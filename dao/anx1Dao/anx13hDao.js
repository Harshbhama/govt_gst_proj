...
async function savetab3HTxn(tab3h, items, gstin, fp, taxperiod) {
  logger.log(
    "info",
  ...
  try {
    ...
    await db.saveTxn(
      anx1Comm.anx1Queries.table3h.save3h,
      [
        tab3h.docref,
        tab3h.ctin,
    ...
    await db.saveTxn(anx1Comm.anx1Queries.itemDetails.saveItemDetailsBatch +
...
async function gettable3HdetailsTxn(gstin , rtnprd , flag) {
  logger.log( "info", "Starting gettable3HdetailsTxn : %s  ::  %s", new Date().getTime(), new Date().toString());
let sql  = (flag !== "F") ? anx1Comm.anx1Queries.table3h.get3hDetails : anx1Comm.anx1Queries.table3h.get3hDetailsErr
...
let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
   ...
   */
  async function editTab3HTxn(tab3H ,items, gstin, flag ) {
    ...
    try {
        ...
        items.forEach((item) => ( _.isUndefined(item.itemid) ? insertItems : updateItems).push(item));
         ...
         await db.saveTxn(
            anx1Comm.anx1Queries.table3h.update3h,
            [tab3H.docref,
            tab3H.ctin,
            tab3H.legaltradename,
            tab3H.pos,
       ...
       let updateRecordCount = await updateItemsTxn(updateItems, gstindb) || 0
        ...
        insertRecordCount = await db.saveTxn(anx1Comm.anx1Queries.itemDetails.saveItemDetailsBatch + prepareItems3H(insertItems),[], gstindb)  
        if(items.length !== (insertRecordCount + updateRecordCount)) { 
          throw new Error(message = "Error in updating Item Details") 
        } 
    
       await save3HSummaryTxn(gstin, tab3H.fp , tab3H.taxperiod, gstindb);  

       if(tab3H.flag == "C" || (flag && flag == "F")){ // Summary to be calculated for error flow - corrected records
          await save3HSummaryTxn(gstin, tab3H.fp , tab3H.taxperiod, gstindb, "F");  
       }
   ...
   * @param {*} rtnprd 
   */
  async function deleteTab3HBydocidsTxn(docIds, gstin, fp, rtnprd, flag , actionFlag) {
    ...
    let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
      ...
      let docIdsResult = await db.fetchAllByIdInTransaction(anx1Comm.anx1Queries.table3h.get3hByDocIds, docIds, gstindb, true);
      ...
      if (docIdsResult.length === 0) {
        ...
        if (diffRec.length === 0) {
          ...
          let deleteRecords = await db.deleteBySqlInClauseTxn(anx1Comm.anx1Queries.table3h.delete3hByDocIds,docIds, gstindb);
  
          await save3HSummaryTxn(gstin, fp, rtnprd,gstindb);

          if(flag && (flag == "F" || flag=="N")){
            await save3HSummaryTxn(gstin, fp, rtnprd, gstindb, flag ,actionFlag);
          }
          db.dbCommit(gstindb)
   ...
   * @param {*} dbName
   */
  async function deleteTab3HByItemIdTxn(itemIds = [], gstin, fp, taxperiod) {
    ...
    try {
      ...
      let row3H = await db.fetchAllByIdTxn(anx1Comm.anx1Queries.table3h.get3hDetailsByDocRef,[item[0].ITEMREF],gstindb);
    ...
    let recordsUpdated = await db.updateOrDeleteTxn(
      anx1Comm.anx1Queries.table3h.update3hDetailsByDocRef,
 ...
 * @param {*} gstindb
 */
async function save3HSummaryTxn(gstin, fp, taxperiod, gstindb, flag , actionFlag) {
    try {
      logger.log("info", "Inside save3HSummaryTxn : %s  ::  %s", gstin, fp);
      /** Calculate Summary object */
     if(flag && flag=="N"){
        if(actionFlag && actionFlag == "C"){
          var summObj3HErr = await db.fetchByIdTxn(anx1Comm.anx1Queries.errorSummary.calculate3HErrorSumm,
        [fp, taxperiod],
        gstindb
      );
      ...
      let summObj3H = await db.fetchByIdTxn(anx1Comm.anx1Queries.summary.calculate3HSumm,
        [fp, taxperiod],
        gstindb
      );
      ...
      if(flag && flag=="N"){
        if(actionFlag && actionFlag == "C"){
          var MarkForDeleteCountErr = await db.getCountTxn(anx1Comm.anx1Queries.errorSummary.getErrorCountfor3H,
        [fp, taxperiod],
        gstindb
      );
        }
      ...
      let MarkForDeleteCount = await db.getCountTxn(anx1Comm.anx1Queries.summary.getCountMarkForDelfor3H,
        [fp, taxperiod],
        gstindb
      );
...
module.exports = {
  ...
  deleteTab3HBydocidsTxn: deleteTab3HBydocidsTxn,
  deleteTab3HByItemIdTxn:deleteTab3HByItemIdTxn,
  gettable3HdetailsTxn:gettable3HdetailsTxn,
  save3HSummaryTxn : save3HSummaryTxn
