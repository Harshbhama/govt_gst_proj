 ...
 * @param {*} gstin 
 */
async function getTab3IAdetails(gstin, rtnprd , flag) {
    try {
      let sql = anx1Comm.anx1aQueries.table3ia.getAll3i;
      if(flag == "F") sql = anx1Comm.anx1aQueries.table3ia.getAll3iErr
...
function save3IASummary(gstin, fp, taxperiod) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      return db.fetchById(anx1Comm.anx1aQueries.table3ia.calculate3iSumm, [fp, taxperiod], gstin)
        .then((rows, err) => {
          if (err) {
            console.log(err);
          ...
          return db.getCount(anx1Comm.anx1aQueries.table3ia.getCountMarkForDelfor3i, [fp, taxperiod], gstin);
        })
        .then((count) => {
          summObj3I.cgst = null;
          summObj3I.sgst = null;
          return save3Aor3HSummary(gstin, fp, taxperiod, count, summObj3I, "3IA");
        })
        .then((isSummarySaved) => {
...
function get3IAjSON(gstin, rtnprd,flag){
  return new Promise(function (resolve, reject) {
    logger.log("info","Inside get3IjSON : %s" , flag);
    let sql = anx1Comm.anx1aQueries.table3ia.get3IjSON;
    if (flag == "F")
      sql = anx1Comm.anx1aQueries.table3ia.get3IErrJson;
...
async function saveTab3IATxn(tab3i ,items, gstin) {
  ...
  try {
           ...
           await db.saveTxn(anx1Comm.anx1aQueries.table3ia.save3i , [
                tab3i.docref ,
                tab3i.pos , 
                tab3i.taxVal , 
                tab3i.igst , 
           ...
           await db.saveTxn(anx1Comm.anx1aQueries.itemDetails.saveItemDetailsBatch + prepareItems3i(items),[], gstindb)       
           await save3IASummaryTxn(gstin , tab3i.fp , tab3i.taxPeriod,gstindb)  
 ...
 * @param {*} flag 
 */
async function editTab3IATxn(tab3i ,items , gstin, fp, taxPeriod, flag) {
  logger.log("info","Starting editTab3IATxn : %s  ::  %s", new Date().getTime(), new Date().toString());
  let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
  ...
  // Segregate Insert and Update Items  based on ItemId undefined
    ...
    await db.saveTxn(anx1Comm.anx1aQueries.table3ia.edit3i , [
      tab3i.docref ,
      tab3i.pos , 
      tab3i.taxVal , 
      tab3i.igst , 
    ...
    let updateRecordCount = await updateItemsTxn(updateItems, gstindb) || 0
      ...
      insertRecordCount = await db.saveTxn(anx1Comm.anx1aQueries.itemDetails.saveItemDetailsBatch + prepareItems3i(insertItems),[], gstindb)  
      ...
      logger.log("info", "save3IASummaryTxn in edit  : %s, %s, %s", gstin, fp, taxPeriod)

      if((flag && flag == "F") || tab3i.flag == "C"){ // Summary to be calculated for error flow - corrected records
        console.log("Saving error summary for 3IA");
        await save3IASummaryTxn(gstin ,fp ,taxPeriod,gstindb, "F")
      }

      db.dbCommit(gstindb)
      
      logger.log("info","Ending editTab3IATxn : %s  ::  %s", new Date().getTime(), new Date().toString());
      return { message: "Document updated successfully", statusCd: STATUS_CD_ONE}
      
  } catch (err) {
    logger.log("info","Error editTab3IATxn : %s  ::  %s", new Date().getTime(), new Date().toString());
     if (err.message.includes("UNIQUE constraint failed")){
 ...
 * @param {*} rtnprd 
 */
async function deleteTab3IABydocidsTxn(docIds, gstin, fp, rtnprd) {
  logger.log("info","Starting deleteTab3IABydocidsTxn : %s  ::  %s", new Date().getTime(), new Date().toString());
  let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
    try {
    ...
    let docIdsResult = await db.fetchAllByIdInTransaction(anx1Comm.anx1aQueries.table3ia.getTab3iByDocIds, docIds, gstindb, true);

    if (docIdsResult.length === 0) {
      logger.log("info","End of deleteTab3IABydocidsTxn : %s  ::  %s", new Date().getTime(), new Date().toString());
      ...
      if (diffRec.length === 0) {
        ...
        let deleteRecords = await db.deleteBySqlInClauseTxn(anx1Comm.anx1aQueries.table3ia.deleteTab3iByDocIds,docIds, gstindb);

        await save3IASummaryTxn(gstin, fp, rtnprd, gstindb);
        await save3IASummaryTxn(gstin, fp, rtnprd, gstindb, "F"); 
        db.dbCommit(gstindb)
 ...
 * @param {*} dbName
 */
async function deleteTab3IAByItemIdTxn(itemIds = [], gstin, fp, taxperiod, flag, status) {
  logger.log("info","Starting deleteTab3IAByItemIdTxn : %s  ::  %s", new Date().getTime(), new Date().toString());
  let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
  ...
  try {
    ...
    let row3I = await db.fetchAllByIdTxn(anx1Comm.anx1aQueries.table3ia.get3iDetailsByDocRef,[item[0].ITEMREF],gstindb);
    ...
    await db.updateOrDeleteTxn(
      anx1Comm.anx1aQueries.table3ia.update3iDetailsByDocRef,[updateItem.TAXVAL, updateItem.IGST, updateItem.CESS, updateItem.DOCREF],gstindb
    );
    
    if(status && status === "F"){
      await db.updateOrDeleteTxn(anx1Comm.anx1aQueries.table3ia.update3iFlagByDocRef, [updateItem.DOCREF], gstindb);  // to update flag as corrected
    } 

    if(flag && flag === "F"){
      await save3IASummaryTxn(gstin, fp, taxperiod,gstindb, flag);   // generate error summary on item delete 
    }

    await save3IASummaryTxn(gstin, fp, taxperiod,gstindb);
    db.dbCommit(gstindb)
    logger.log("info","Ending deleteTab3IAByItemIdTxn : %s  ::  %s", new Date().getTime(), new Date().toString());
...
async function save3IASummaryTxn(gstin, fp, taxperiod, gstindb, flag) {
  try {
    logger.log("info", "Inside save3IASummaryTxn for  : %s, %s, %s", gstin, fp, taxperiod)
    /** Calculate Summary object */
    let summObj3I = await db.fetchByIdTxn((flag && flag == "F" ? anx1Comm.anx1aQueries.errorSummary.calculate3iErrSumm : anx1Comm.anx1aQueries.table3ia.calculate3iSumm), [fp, taxperiod], gstindb)
    
    /** get the count for mark for delete */
    let MarkForDeleteCount = await db.getCountTxn((flag && flag == "F" ? anx1Comm.anx1aQueries.errorSummary.getYetToBeCorrectedCount3i : anx1Comm.anx1aQueries.table3ia.getCountMarkForDelfor3i), [fp, taxperiod], gstindb)
    ...
    let sumarrySaved = await save3Aor3HSummaryTxn(gstin, fp, taxperiod, MarkForDeleteCount, summObj3I, "3IA", gstindb, flag)
...
module.exports={
    ...
    save3IASummaryTxn :save3IASummaryTxn
}
