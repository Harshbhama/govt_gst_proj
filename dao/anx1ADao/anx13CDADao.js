...
const {saveItemDetails3H , updateItemDetails, updateItemDetails3H} = require('../anx1Dao')
const _ = require("lodash");
const {save3CDSummary,save3CDSummaryTxn} = require('../anx1SummaryDao');
const {sendResponse} = require('../../utility/common');
const logger = require('../../utility/logger').logger;


function save3cda(headerList ,fullItemList , gstin){
    return new Promise((resolve, reject) => {
        let docref  = headerList.docref;
        db.save(
          anx1aQueries.table3CDA.save3cdA,
          [ headerList.doctyp,
            gstin,
            headerList.docref,
            headerList.num,
    ...
    async function edit3cda(headerList ,fullItemList , gstin , fp , taxperiod) {
      let docref  = headerList.docref;
      try {
        
        let update3cd = await db.update(
          anx1aQueries.table3CDA.update3cd,
          [ 
            headerList.doctyp,
            headerList.docref,
            headerList.num,
   ...
   async function edit3cdaTxn(edit3cd, items, gstin, fp, taxperiod , flag) {
    ...
    items.forEach((item) => ( _.isUndefined(item.itemid) ? insertItems : updateItems).push(item));
      ...
      let update3cd = await db.update(
        ...
        [ edit3cd.odocref,
          ...
          edit3cd.odocnum,
          edit3cd.odocdate,
          edit3cd.odocdate.substr(edit3cd.odocdate.length - 4),
          edit3cd.doctyp,
          edit3cd.docref,
          edit3cd.num,
          edit3cd.dt,
        ...
        insertRecordCount = await db.saveTxn(anx1aQueries.itemDetails.saveItemDetailsBatch + prepareItems3cd(insertItems),[], gstindb)  
...
async function get3CDADetailsDao(rtnprd, dbName, flag) {
  ...
  try {
    ...
    */
   let sql = anx1aQueries.table3CDA.get3cdAByReturnPeriod
   if(flag == "F") sql  = anx1aQueries.table3CDA.get3cdAByReturnPeriodErr
...
// Deleteall 3hdetails
async function delete3cdaBydocIds(docIds, dbName, fp, rtnprd) {
  try {
    
    // Async db call to check number of items available
    let docIdsResult = await db.fetchAllById(
      anx1aQueries.table3CDA.get3cdAByDocIds,
      docIds,
      dbName,
      true
    );
    ...
    if (docIdsResult.length === 0) {
      ...
      if (diffRec.length === 0) {
        ...
        let deleteRecords = await db.deleteBySqlInClause(
          anx1aQueries.table3cd.delete3cdByDocIds,
          docIds,
          dbName
        );
 ...
 * @param {*} dbName
 */
async function delete3cdaItemsByItemId(itemIds = [], dbName, fp, taxperiod) {
  // try catch block to handle error scenario
  try {
    /* DB call to check number of items available for the given itemids
    slight modification to the fetchAllById method which passing a last argumrnt
   ...
   let deleteRecords = 0
    let item = await db.fetchAllById(
      anx1aQueries.itemDetails.getItemRefByItemId,
      itemIds,
      dbName,
      true
    );
    ...
    let listOfItems = await db.fetchAllById(
      anx1aQueries.itemDetails.getItemsByRefId,
      [item[0].ITEMREF],
      dbName
    );
    ...
    if (listOfItems.length > 1) {
      ...
      deleteRecords = await db.deleteBySqlInClause(
        anx1aQueries.itemDetails.deleteItemsByItemIds,
        itemIds,
        dbName 
      );
    ...
    let row3CD = await db.fetchAllById(
      anx1aQueries.table3CDA.get3cdADetailsByDocRef,
      [item[0].ITEMREF],
      dbName
    );
    ...
    let recordsUpdated = await db.update(
      anx1aQueries.table3CDA.update3cdADetailsByDocRef,
      [u.TAXVAL, u.IGST, u.CESS, u.DOCREF],
      dbName
    );
    
  db.fetchById(anx1aQueries.table3CDA.get3cdADetailsByDocRef , [u.DOCREF] , dbName)
  .then(data =>{
 ...
 * @param {*} dbName
 */
async function delete3cdaItemsByItemIdTxn(itemIds = [], gstin, fp, taxperiod , flag , status) {
  ...
  try {
    ...
    if (listOfItems.length > 1) {
      // delete the record from item table only if it has more than one records
      deleteRecords = await db.deleteBySqlInClauseTxn( anx1aQueries.itemDetails.deleteItemsByItemIds,
        itemIds,gstindb);
    ...
    // fetch row from 3CD table based on docRef
    let row3CD = await db.fetchAllByIdTxn(anx1aQueries.table3CDA.get3cdADetailsByDocRef, [item[0].ITEMREF], gstindb);
    ...
    // update record DB call
    let recordsUpdated = await db.updateOrDeleteTxn(anx1aQueries.table3CDA.update3cdADetailsByDocRef
      , [u.TAXVAL, u.IGST, u.CESS, u.DOCREF]
      , gstindb
    );
    if(status && status === "F"){
      await db.updateRow(anx1aQueries.table3CDA.update3cdAFlagByDocRef, [u.DOCREF], gstindb);  // to update flag as corrected
    } 
...
function getDetailsByExportType(rtnprd ,exptyp ,gstin) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    db.fetchAllById(anx1aQueries.table3CDA.getDetailsByExpTyp,
          [rtnprd ,  exptyp],   gstin)
      .then(resultSet => {
...
async function save3cdaTxn(tab3cd ,items , gstin) {
  logger.log("info","Starting save3cd : %s  ::  %s", new Date().getTime(), new Date().toString());
  let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
  try {
           gstindb.exec('BEGIN TRANSACTION;'); 

           await db.saveTxn(anx1aQueries.table3CDA.save3cdA,
            [ tab3cd.odocref,
              tab3cd.odoctyp,
              tab3cd.odocnum,
              tab3cd.odocdate,
              tab3cd.odocdate.substr(tab3cd.odocdate.length - 4),
              tab3cd.doctyp,
              gstin,
              tab3cd.docref,
              tab3cd.num,
           ...
           await db.saveTxn(anx1aQueries.itemDetails.saveItemDetailsBatch + prepareItems3cd(items),[], gstindb)       
           
           // await save3CDSummaryTxn(gstin, tab3cd.fp, tab3cd.taxperiod , tab3cd.exptype , tab3cd.doctyp,gstindb);
           // let markForDelete3cdCount = await db.getCountTxn(anx1aQueries.summary.getCountMarkForDelfor3CD ,[tab3cd.fp , tab3cd.taxperiod , tab3cd.exptype , tab3cd.doctyp] , gstindb)
...
async function delete3cdaBydocIdsTxn(docIds, gstin, fp, rtnprd , flag , actionFlag) {
  logger.log("info","Starting save3cd : %s  ::  %s", new Date().getTime(), new Date().toString());
    let gstindb = new sqlite3.Database(DB_PATH + gstin.toUpperCase() + DB_EXT);
    try {
             gstindb.exec('BEGIN TRANSACTION;'); 
    ...
    if (docIdsResult.length === 0) {
      ...
      if (diffRec.length === 0) {
        ...
        let deleteRecords = await db.deleteBySqlInClauseTxn(
          anx1aQueries.table3CDA.delete3cdAByDocIds,
          docIds,
          gstindb 
               );
...
async function summary3cdTxn(fp, taxperiod, gstindb, isMFD = false) {
  try {
    let returnValue = 1
    await db.saveTxn(anx1aQueries.summary.delete3Aor3Hsumm, [fp, taxperiod, "3CDA"], gstindb);
    let mfd = await db.fetchAllByIdTxn(anx1aQueries.summary.checkMarkForDelete3cd, [fp, taxperiod], gstindb)
    // let mai = await db.fetchAllByIdTxn(anx1aQueries.summary.checkMarkInvalid3cd, [fp, taxperiod], gstindb)
    // console.log('MAI ', mai);
    await db.saveTxn(anx1aQueries.summary.docTypeValuesInsertionFor3cd, [fp, taxperiod], gstindb);
    if (mfd.length === 0 && isMFD) {
      console.log('MFD IF');
      returnValue
    } else {
      console.log('MFD ELSE');
      await db.saveTxn(anx1aQueries.summary.tableSummarywpAndwopNet, [fp, taxperiod, fp, taxperiod], gstindb);
      await db.saveTxn(anx1aQueries.summary.consolidateNetForDocTypes, [fp, taxperiod], gstindb);
      let row = await db.fetchByIdTxn(anx1aQueries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
      if (row) {
        await db.saveTxn(anx1aQueries.summary.consolidateNet, [fp, taxperiod, fp, taxperiod], gstindb)
      }
    }
    // if (mai.length === 0 && isMAI) {
    //   console.log('MAI IF');
    //   returnValue
    // } else {
    //   console.log('MAI ELSE');
    //   await db.saveTxn(anx1aQueries.summary.tableSummarywpAndwopNet, [fp, taxperiod, fp, taxperiod], gstindb);
    //   await db.saveTxn(anx1aQueries.summary.consolidateNetForDocTypes, [fp, taxperiod], gstindb);
    //   let row = await db.fetchByIdTxn(anx1aQueries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
    //   if (row) {
  ...
  try {
    ...
    if(flag && flag =="N"){ // To update summary when corrected record is deleted in main flow
      if(actionFlag && actionFlag =="C"){
        await db.saveTxn(anx1aQueries.errorSummary.delete3Aor3Hsumm, [fp, taxperiod, "3CDA"], gstindb);
        var mfd = await db.fetchAllByIdTxn(anx1aQueries.errorSummary.getYetToBeCorrectedCount3cd, [fp, taxperiod], gstindb)
        await db.saveTxn(anx1aQueries.errorSummary.docTypeValuesInsertionFor3cd, [fp, taxperiod], gstindb);
        if (mfd.length === 0 && isMFD) {
          returnValue
        } else {
          
          await db.saveTxn(anx1aQueries.errorSummary.tableSummarywpAndwopNet, [fp, taxperiod, fp, taxperiod], gstindb);
          //await db.saveTxn(anx1aQueries.errorSummary.consolidateNetForDocTypes, [fp, taxperiod], gstindb);
          let row = await db.fetchByIdTxn(anx1aQueries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
          if (row) {
            await db.saveTxn(anx1aQueries.errorSummary.consolidateNet, [fp, taxperiod, fp, taxperiod], gstindb)
          }
        }
      }
    }
      ...
      if (mfd.length === 0 && isMFD) {
        ...
        await db.saveTxn(anx1aQueries.errorSummary.tableSummaryAwpAndwopNet, [fp, taxperiod, fp, taxperiod], gstindb);
        //await db.saveTxn(anx1aQueries.errorSummary.consolidateNetForDocTypes, [fp, taxperiod], gstindb);
        let row = await db.fetchByIdTxn(anx1aQueries.table3CDA.get3cdAByFPAndTaxperiod, [fp, taxperiod], gstindb);
        if (row) {
          await db.saveTxn(anx1aQueries.errorSummary.consolidateNet, [fp, taxperiod, fp, taxperiod], gstindb)
...
/* async function updateMarkforDeleteCount(mfd,gstindb) {
  try {
    return Promise.all( await mfd.map(async (row) => {
      ...
      let recordsUpdated = await db.updateOrDeleteTxn(anx1aQueries.summary.updateMarkForDeleteFor3cd
...
module.exports={
    save3cda : save3cda, 
    edit3cda:edit3cda,
    get3CDADetailsDao: get3CDADetailsDao,
